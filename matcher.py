...
def execute_system(input_file, output_file, labels_file, evaluation_file):
    ...
    It then creates a text file with the count of true positive, false negative and false
    positives together with precision, recall and F1 score. Note that these values are
    all computed using perfect match.
    ...
    #all matching strings will be stored in the output_list
    output_list = match_all(text_input)
    #sorting the output_list makes the manual comparison of output file and ground truth
    #easier and faster
    #this is very useful during the development of the program when new regex are
    #developped by trial and error (ie by catching dates that were missed)
    output_list.sort()
    ...
    #partial match evaluation will be done manually        
...
def main():
    ...
    #definitely not a requirement for the program to work, which explains why they
    ...
    #the performance will be written to a file using the specified name
